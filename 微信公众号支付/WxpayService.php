...
class WxpayService
{
    ...
    public function Parameters($order,$money){

        $openId = $this->GetOpenid();      //获取openid
        if(!$openId) exit('获取openid失败');
        //②、统一下单
        $outTradeNo = $order->id."_".uniqid();     //你自己的商品订单号
        $payAmount = $money;          //付款金额，单位:元
        $orderName = $order->product->title;    //订单标题
        $notifyUrl = 'http://fupin.goturan.com/index/Order/notify';     //付款成功后的回调地址(不要有问号)
        $payTime = time();      //付款时间
    ...
    public function GetOpenid()
    {
        ...
        //     //触发微信返回code码
        //     $scheme = 'http://';
        //     $baseUrl = $scheme.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING'];
